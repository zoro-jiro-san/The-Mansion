...
- **Coordinates with Ant Farm lanes**: Provides skills to each Ant Farm lane, ensures skill distribution across lanes, monitors skill usage per lane
- Coordinates with Butler for resource alignment (skills + API keys per lane)
...
- **Coordinates Ant Farm lanes**: Manages all Ant Farm lanes, tracks progress per lane, coordinates resources per lane
- Monitors all sub-agents continuously (organized in Ant Farm lanes)
- **Coordinates with Librarian**: Ensures skills are distributed appropriately across Ant Farm lanes
- **Coordinates with Butler**: Ensures API keys are allocated appropriately across Ant Farm lanes, receives usage data per lane
...
5. **Resource Coordination** (Factory Manager coordinates with specialists):
   - **Discusses with Librarian**: Factory Manager discusses task requirements with Librarian to determine what skills agents will need for each Ant Farm lane
   - **Requests API keys from Butler**: Factory Manager requests API keys from Butler for each Ant Farm lane, Butler assesses the work and assigns appropriate secondary API keys per lane
   - **Librarian provides skills**: Librarian matches skills to task requirements and provides them for each Ant Farm lane
   - **Ant Farm Coordination**: Factory Manager coordinates with Moderator, Librarian, and Butler to set up Ant Farm lanes
6. **Agent Spawning in Ant Farm**: Factory Manager creates sub-agent(s) organized in Ant Farm lanes with:
   - Complete PRD
...
**CRITICAL**: Sub-agents MUST be organized in Ant Farm lanes. This is not optional - it's a core architectural requirement.

**1. Lane Creation**:
- Factory Manager creates parallel execution lanes based on task analysis
- **Sub-agents are organized into these Ant Farm lanes** (this is mandatory)
- Each lane is independent and can run concurrently
- Lanes are assigned to different teams of sub-agents
- Each lane has its own API keys (from Butler, coordinated) and skills (from Librarian, coordinated)
- **Coordination**: Moderator, Librarian, and Butler coordinate with Ant Farm lanes
...
**3. Lane Management & Coordination**:
- **Moderator**: Manages all Ant Farm lanes simultaneously, tracks progress per lane, handles blockers per lane
- **Librarian**: Coordinates skill distribution across Ant Farm lanes, ensures each lane has appropriate skills
- **Butler**: Coordinates API key allocation across Ant Farm lanes, monitors usage per lane, rotates keys per lane
- All three (Moderator, Librarian, Butler) work together to coordinate Ant Farm lanes
- Tracks progress for each lane independently
- Coordinates resource sharing between lanes
...
**CRITICAL**: Sub-agents MUST be organized in Ant Farm lanes. All coordination happens through Moderator, Librarian, and Butler.

- **Factory Manager**: Creates Ant Farm lanes and assigns teams to lanes
- **Butler**: Allocates API keys to each Ant Farm lane, monitors usage per lane, rotates keys per lane
- **Librarian**: Provides skills to each Ant Farm lane's agents, ensures skill distribution across lanes
- **Moderator**: Monitors all Ant Farm lanes, coordinates resources per lane, manages sub-agents in lanes
- **Gatekeeper**: Reviews code from all Ant Farm lanes
- **Janitor**: Cleans up after all Ant Farm lanes complete

**Coordination Flow**:
1. Factory Manager creates Ant Farm lanes with sub-agents
2. Moderator coordinates overall lane management
3. Librarian coordinates skill distribution across lanes
4. Butler coordinates API key allocation across lanes
5. All three (Moderator, Librarian, Butler) work together to ensure lanes operate efficiently
...
8. **Factory Manager** → **Discusses with Librarian** about what skills agents will need for each Ant Farm lane
9. **Librarian** → Provides skill recommendations based on task requirements for each lane
10. **Factory Manager** → Requests API keys from **Butler** for each Ant Farm lane
11. **Butler** → **Assesses the work** and assigns appropriate secondary API keys per Ant Farm lane
12. **Librarian** → Provides matched skills to Factory Manager for each Ant Farm lane
13. **Factory Manager** → Creates teams if needed and assigns them to Ant Farm lanes (Factory Manager handles ALL team creation)
14. **Factory Manager** → Spawns sub-agent(s) organized in Ant Farm lanes with:
    - PRD
    - API key per lane (from Butler, who assessed the work)
    - Required skills per lane (from Librarian, after discussion)
    - Lane assignment
15. **Factory Manager** → Hands off Ant Farm lanes to **Moderator** for coordination
16. **Moderator, Librarian, Butler** → Coordinate Ant Farm lanes (Moderator manages, Librarian provides skills, Butler manages API keys)

### Phase 3: Task Execution in Ant Farm Lanes
17. **Moderator** → Begins monitoring all Ant Farm lanes and sub-agents
18. **Sub-Agents in Ant Farm lanes** → Start working on tasks in parallel lanes using:
    - Assigned API key per lane (from Butler, coordinated)
    - Assigned skills per lane (from Librarian, coordinated)
    - PRD guidance (from Factory Manager)
19. **Sub-Agents** → Report progress to Moderator (per lane)
20. **Moderator** → Tracks progress per Ant Farm lane and checks for blockers
21. **Moderator, Librarian, Butler** → Continuously coordinate Ant Farm lanes:
    - Moderator: Manages lanes and sub-agents
    - Librarian: Monitors and provides skills per lane
    - Butler: Monitors and manages API keys per lane
